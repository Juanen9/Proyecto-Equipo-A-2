...
const recoverPassword = async(req, res) => {
    try {
        ...
        if(!email) return res.status(400).send('Debe ingresar email para recuperar la contraseña.');
        ...
        if(user.length === 0) return res.status(404).send('No existe un usuario con ese correo electrónico.');
